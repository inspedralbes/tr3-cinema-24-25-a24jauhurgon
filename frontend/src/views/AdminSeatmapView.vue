  ...
  <div class="space-y-6 animate-fade-in-up">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-sm text-slate-400" v-if="vol && vol.modelAvio">Bcn ➔ {{ vol.destiIata }} | Avió: {{
            vol.modelAvio.nomModel }}</p>
        </div>
      </div>
    ...
    <div v-if="vol && !carregant" class="flex flex-col lg:flex-row gap-8">
      ...
      <div class="lg:w-2/3 glass-panel p-8 rounded-xl flex flex-col items-center overflow-x-auto relative">
        <div
          class="w-24 h-32 border-4 border-slate-700/50 rounded-t-full mb-8 relative flex items-center justify-center bg-slate-800/20">
              ...
              <template v-for="col in vol.modelAvio.columnes" :key="col">
                ...
                <div class="relative group/seat w-10 h-10">
                  <template v-if="!(fila <= 3 && (col === 2 || col === 5))">
                    <div
                      class="w-10 h-10 rounded text-xs font-bold flex items-center justify-center transition-all duration-300 border shadow-sm cursor-help relative overflow-hidden"
                      :class="{
                      ...
                      <span v-if="estatSeient(fila, col).tipus === 'embarcat'"
                        class="material-icons text-[18px]">how_to_reg</span>
                      <span v-else-if="estatSeient(fila, col).tipus === 'comprat'"
                        class="material-icons text-[16px]">check</span>
                      <span v-else-if="estatSeient(fila, col).tipus === 'bloquejat'"
                        class="material-icons text-[16px]">shopping_cart</span>
      ...
      <div class="lg:w-1/3 space-y-6">
        <div class="glass-panel p-6 rounded-xl border border-slate-700/50">
          ...
          <div class="space-y-4">
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded bg-slate-800 border border-slate-700 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded bg-orange-500 border border-orange-400 flex flex-col items-center justify-center relative overflow-hidden shadow-orange-500/20 shadow-lg">
            ...
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded bg-green-500 border border-green-400 flex flex-col items-center justify-center relative overflow-hidden shadow-green-500/20 shadow-lg">
            ...
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded bg-cyan-500 border border-cyan-400 flex flex-col items-center justify-center relative overflow-hidden shadow-cyan-500/40 shadow-lg">
        ...
        <div class="glass-panel p-6 rounded-xl border border-slate-700/50" v-if="vol.modelAvio">
          <div class="flex justify-between items-end mb-2">
            ...
            <span class="text-2xl font-bold text-white">{{ seientsComprats.length + seientsBloquejats.length }}<span
                class="text-sm text-slate-500">/{{ vol.modelAvio.seientsTotals }}</span></span>
          </div>
          <div class="w-full bg-slate-800 rounded-full h-2 mb-4 overflow-hidden flex">
            <!-- Bloquejats -->
            <div class="bg-orange-500 h-2 transition-all duration-500"
              :style="`width: ${(seientsBloquejats.length / vol.modelAvio.seientsTotals) * 100}%`"></div>
            <!-- Comprats -->
            <div class="bg-green-500 h-2 transition-all duration-500"
              :style="`width: ${(seientsComprats.length / vol.modelAvio.seientsTotals) * 100}%`"></div>
          </div>
        </div>
      </div>
    </div>
